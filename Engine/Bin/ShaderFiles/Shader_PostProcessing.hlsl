...
Texture2D<float4> NonBlurTexture : register(t3);
Texture2D<float4> BlurTexture : register(t4);
Texture2D<float4> DistortionTexture : register(t5);
...
PS_OUT PS_MAIN(PS_IN In)
{
	...
	float3 vEffectColor = saturate(vNonBlur + vBlur);
