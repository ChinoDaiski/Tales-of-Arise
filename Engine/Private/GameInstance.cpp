...
CGameObject * CGameInstance::Clone_Prototype(const _tchar * pPrototypeTag, void * pArg)
{
	if (nullptr == m_pObject_Manager)
		return nullptr;

	return m_pObject_Manager->Clone_Prototype(pPrototypeTag, pArg);
}
...
void CGameInstance::LoadDataFile_Effect()
{
	...
	//fire_avatar
	hFile = CreateFile(L"../../Client/Bin/data/EffectData/FireAvatar/effect.dat", GENERIC_READ, 0, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	if (0 == hFile)
		return;
	...
	while (true)
	{
		...
		InstanceEffectDesc_FireAvatar.push_back(vTemp);
	}
	CloseHandle(hFile);
}
EFFECTDESC * CGameInstance::Get_InstanceEffect_Data(UNIT_TYPE type, _int index)
{
	...
	case Engine::UNIT_FIREAVATAR:
		if (index < InstanceEffectDesc_FireAvatar.size())
		{
			return &InstanceEffectDesc_FireAvatar[index];
			break;
		}
