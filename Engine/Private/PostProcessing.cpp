...
_bool CPostProcessing::Get_DissolveStart()
{
...
void CPostProcessing::Set_DissolveStart(_bool bStart)
{
...
HRESULT CPostProcessing::Combine_Resources(CVIBuffer_Rect* pVIBuffer, _float4x4* pMatrix)
{
	...
	ID3D11RenderTargetView* pRTV[2] = { nullptr };
	ID3D11DepthStencilView* pDSV = nullptr;

	m_pDeviceContext->OMGetRenderTargets(2, pRTV, &pDSV);
...
HRESULT CPostProcessing::DownScale()
{
	...
	if (FAILED(m_pShader_PostProcessing_FX->Set_ShaderResourceView("NonBlurTexture", m_pTarget_Manager->Get_SRV(L"Target_NonBlur"))))
		return E_FAIL;
...
HRESULT CPostProcessing::FinalPass(CVIBuffer_Rect* pVIBuffer, _float4x4* pMatrix)
{
	...
	if (FAILED(m_pShader_PostProcessing_FX->Set_ShaderResourceView("BlurTexture", m_pDownScale4x4SRV[BLUR])))
		return E_FAIL;
	if (FAILED(m_pShader_PostProcessing_FX->Set_ShaderResourceView("DistortionTexture", m_pDownScale4x4SRV[DISTORTION])))
		return E_FAIL;
	...
	_uint iShaderPass = 0;
	if (true == m_bDissolveStart)
	{
		iShaderPass = 1;
	}
	m_pShader_PostProcessing_FX->Begin(iShaderPass);
	pVIBuffer->Render();
...
HRESULT CPostProcessing::SetUp_Texture_Buffer()
{
	...
	if (FAILED(m_pDevice->CreateTexture2D(&tTextureDesc, nullptr, &m_pDownScale4x4RTV[OBJECTGLOW])))
		return E_FAIL;
	if (FAILED(m_pDevice->CreateShaderResourceView(m_pDownScale4x4RTV[OBJECTGLOW], &tSRVDesc, &m_pDownScale4x4SRV[OBJECTGLOW])))
		return E_FAIL;
	if (FAILED(m_pDevice->CreateUnorderedAccessView(m_pDownScale4x4RTV[OBJECTGLOW], &tUAVDesc, &m_pDownScale4x4UAV[OBJECTGLOW])))
		return E_FAIL;

	if (FAILED(m_pDevice->CreateTexture2D(&tTextureDesc, nullptr, &m_pDownScale4x4RTV[DISTORTION])))
		return E_FAIL;
	if (FAILED(m_pDevice->CreateShaderResourceView(m_pDownScale4x4RTV[DISTORTION], &tSRVDesc, &m_pDownScale4x4SRV[DISTORTION])))
		return E_FAIL;
	if (FAILED(m_pDevice->CreateUnorderedAccessView(m_pDownScale4x4RTV[DISTORTION], &tUAVDesc, &m_pDownScale4x4UAV[DISTORTION])))
		return E_FAIL;
