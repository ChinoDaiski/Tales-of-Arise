...
HRESULT CRenderer::NativeConstruct_Prototype()
{
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : nullptr == m_pTarget_Manager", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->Create_DSV(m_pDevice, g_iLightCX, g_iLightCY)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Create_DSV", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Output"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.f, 0.f, 1.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Output)", E_FAIL);
	/* For.Target_Dissolve */
	...
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Diffuse"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_B8G8R8A8_UNORM, _float4(1.f, 1.f, 1.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Diffuse)", E_FAIL);
	/* For.Target_Normal */
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Normal"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R16G16B16A16_UNORM, _float4(1.f, 1.f, 1.f, 1.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Normal)", E_FAIL);
	/* For.Target_Depth */
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Depth"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(1.f, 1.f, 1.f, 1.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Depth)", E_FAIL);
	/* For.Target_Emissive */
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Emissive"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.f, 0.f, 0.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Emissive)", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_ObjectGlow"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.f, 0.f, 0.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_ObjectGlow)", E_FAIL);
	/* For.Target_NonBlur */
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_NonBlur"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.f, 0.f, 0.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_NonBlur)", E_FAIL);
	/* For.Target_Blur */
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Blur"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(0.f, 0.f, 0.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Blur)", E_FAIL);
	/* For.Target_Shade */
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Shade"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_B8G8R8A8_UNORM, _float4(0.f, 0.f, 0.f, 1.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Shade)", E_FAIL);
	/* For.Target_Specular */
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Specular"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_B8G8R8A8_UNORM, _float4(0.f, 0.f, 0.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Specular)", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_LightDepth"), m_pDevice, m_pDeviceContext, g_iLightCX, g_iLightCY, DXGI_FORMAT_R32G32B32A32_FLOAT, _float4(1.f, 1.f, 1.f, 1.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_LightDepth)", E_FAIL);
	/* For.Target_HDR */
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_HDR"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R16G16B16A16_FLOAT, _float4(0.f, 0.f, 0.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_HDR)", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->Add_RenderTarget(TEXT("Target_Distortion"), m_pDevice, m_pDeviceContext, (_uint)Viewport.Width, (_uint)Viewport.Height, DXGI_FORMAT_R16G16B16A16_FLOAT, _float4(0.f, 0.f, 0.f, 0.f))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_RenderTarget(Target_Distortion)", E_FAIL);

	/* For.MRT_Final */
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_Final"), TEXT("Target_Output"))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_MRT(MRT_Final)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_Final"), TEXT("Target_Dissolve"))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_MRT(MRT_Final)", E_FAIL);

	/* For.MRT_Deferred */
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_Deferred"), TEXT("Target_Diffuse"))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_MRT(MRT_Deferred)", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_LightAcc"), TEXT("Target_Shade"))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_MRT(MRT_LightAcc)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_LightAcc"), TEXT("Target_Specular"))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_MRT(MRT_LightAcc)", E_FAIL);

	/* For.MRT_LightDepth */
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_LightDepth"), TEXT("Target_LightDepth"))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_MRT(MRT_LightDepth)", E_FAIL);

	/* For.MRT_HDR */
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_HDR"), TEXT("Target_HDR"))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_MRT(MRT_HDR)", E_FAIL);

	/* For.MRT_Effect */
	if (FAILED(m_pTarget_Manager->Add_MRT(TEXT("MRT_Effect"), TEXT("Target_NonBlur"))))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Add_MRT(MRT_Effect)", E_FAIL);
	...
	if (nullptr == m_pVIBuffer)
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : nullptr == m_pVIBuffer", E_FAIL);
	...
	if (nullptr == m_pShader)
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : nullptr == m_pShader", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->Ready_DebugBuffer(m_pDevice, m_pDeviceContext)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugBuffer", E_FAIL);
	
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Diffuse"), 0.f, 0.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Diffuse)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Normal"), 0.f, 150.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Normal)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Depth"), 0.f, 300.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Depth)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Emissive"), 0.f, 450.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Emissive)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_ObjectGlow"), 0.f, 600.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_ObjectGlow)", E_FAIL);

	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Shade"), 150.f, 0.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Shade)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Specular"), 150.f, 150.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Specular)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_LightDepth"), 150.f, 300.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_LightDepth)", E_FAIL);

	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_HDR"), Viewport.Width - 150.f, 0.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_HDR)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Output"), Viewport.Width - 150.f, 150.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Output)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Dissolve"), Viewport.Width - 150.f, 300.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Dissolve)", E_FAIL);

	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_NonBlur"), Viewport.Width - 300.f, 0.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_NonBlur)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Blur"), Viewport.Width - 300.f, 150.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Blur)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Ready_DebugDesc(TEXT("Target_Distortion"), Viewport.Width - 300.f, 300.f, 150.f, 150.f)))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : Ready_DebugDesc(Target_Distortion)", E_FAIL);

	if (FAILED(SetUp_RenderState()))
		CHECK_MSG(L"Failed To CRenderer : NativeConstruct_Prototype : SetUp_RenderState", E_FAIL);
#endif // _DEBUG
...
HRESULT CRenderer::Add_RenderGroup(RENDERGROUP eGroup, CGameObject* pGameObject)
{
	if (nullptr == pGameObject)
		CHECK_MSG(L"Failed To CRenderer : Add_RenderGroup : nullptr == pGameObject", E_FAIL);
...
HRESULT CRenderer::Render_GameObjects()
{
	if(FAILED(m_pTarget_Manager->Clear_MRT(m_pDeviceContext, L"MRT_HDR")))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Clear_MRT(MRT_HDR)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Clear_MRT(m_pDeviceContext, L"MRT_Effect")))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Clear_MRT(MRT_Effect)", E_FAIL);
	if (FAILED(m_pTarget_Manager->Clear_RenderTarget(L"Target_Output")))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Clear_RenderTarget(Target_Output)", E_FAIL);

	if (FAILED(Render_Priority()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Priority", E_FAIL);
	if (FAILED(Render_LightDepth()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_LightDepth", E_FAIL);
	if (FAILED(Render_NonBlend()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_NonBlend", E_FAIL);
	if (FAILED(Render_LightAcc()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_LightAcc", E_FAIL);
	if (FAILED(Render_Deferred()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Deferred", E_FAIL);
	if (FAILED(Render_NonLight()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_NonLight", E_FAIL);
	if (FAILED(Render_Blend()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Blend", E_FAIL);
	if (FAILED(Render_Result()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Result", E_FAIL);
	if (FAILED(Render_Output()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Output", E_FAIL);
	if (FAILED(Render_UI()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_UI", E_FAIL);

#ifdef _DEBUG

	if (FAILED(Render_DebugComponents()))
		CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_DebugComponents", E_FAIL);
	
	if (true == g_bDeferred)
	{
		if (FAILED(m_pTarget_Manager->Render_Debug(TEXT("MRT_Deferred"))))
			CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Debug(MRT_Deferred)", E_FAIL);
	}
	if (true == g_bLightAcc)
	{
		if (FAILED(m_pTarget_Manager->Render_Debug(TEXT("MRT_LightAcc"))))
			CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Debug(MRT_LightAcc)", E_FAIL);
	}
	if (true == g_bLightDepth)
	{
		if (FAILED(m_pTarget_Manager->Render_Debug(TEXT("MRT_LightDepth"))))
			CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Debug(MRT_LightDepth)", E_FAIL);
	}
	if (true == g_bFinal)
	{
		if (FAILED(m_pTarget_Manager->Render_Debug(TEXT("MRT_Final"))))
			CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Debug(MRT_Final)", E_FAIL);
	}
	if (true == g_bHDR)
	{
		if (FAILED(m_pTarget_Manager->Render_Debug(TEXT("MRT_HDR"))))
			CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Debug(MRT_HDR)", E_FAIL);
	}
	if (true == g_bEffect)
	{
		if (FAILED(m_pTarget_Manager->Render_Debug(TEXT("MRT_Effect"))))
			CHECK_MSG(L"Failed To CRenderer : Render_GameObjects : Render_Debug(MRT_Effect)", E_FAIL);
	}
...
{	
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_Priority : nullptr == m_pTarget_Manager", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_NonClear_MRT(m_pDeviceContext, TEXT("MRT_HDR"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Priority : Begin_NonClear_MRT(MRT_HDR)", E_FAIL);
	...
	for (auto& pGameObject : m_RenderList[GROUP_PRIORITY])
	{
		...
		if (FAILED(hr))
			CHECK_MSG(L"Failed To CRenderer : Render_Priority : hr", E_FAIL);
	}
	m_RenderList[GROUP_PRIORITY].clear();

	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_HDR"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Priority : End_MRT(MRT_HDR)", E_FAIL);
...
{
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_Terrain : nullptr == m_pTarget_Manager", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_NonClear_MRT(m_pDeviceContext, TEXT("MRT_HDR"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Terrain : Begin_NonClear_MRT(MRT_HDR)", E_FAIL);
	...
	for (auto& pGameObject : m_RenderList[GROUP_TERRAIN])
	{
		...
		if (FAILED(hr))
			CHECK_MSG(L"Failed To CRenderer : Render_Terrain : hr", E_FAIL);
	}
	m_RenderList[GROUP_TERRAIN].clear();

	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_HDR"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Terrain : End_MRT(MRT_HDR)", E_FAIL);
...
{
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_LightDepth : nullptr == m_pTarget_Manager", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_LightDepth_MRT(m_pDeviceContext, TEXT("MRT_LightDepth"))))
		CHECK_MSG(L"Failed To CRenderer : Render_LightDepth : Begin_LightDepth_MRT(MRT_LightDepth)", E_FAIL);
	...
	for (auto& pGameObject : m_RenderList[GROUP_LIGHTDEPTH])
	{
		...
		if (FAILED(hr))
			CHECK_MSG(L"Failed To CRenderer : Render_LightDepth : hr", E_FAIL);
	}
	m_RenderList[GROUP_LIGHTDEPTH].clear();

	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_LightDepth"))))
		CHECK_MSG(L"Failed To CRenderer : Render_LightDepth : End_MRT(MRT_LightDepth)", E_FAIL);
...
{
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_NonBlend : nullptr == m_pTarget_Manager", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_MRT(m_pDeviceContext, TEXT("MRT_Deferred"))))
		CHECK_MSG(L"Failed To CRenderer : Render_NonBlend : Begin_MRT(MRT_Deferred)", E_FAIL);
	...
	for (auto& pGameObject : m_RenderList[GROUP_NONBLEND])
	{
		...
		if (FAILED(hr))
			CHECK_MSG(L"Failed To CRenderer : Render_NonBlend : hr", E_FAIL);
	}
	m_RenderList[GROUP_NONBLEND].clear();

	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_Deferred"))))
		CHECK_MSG(L"Failed To CRenderer : Render_NonBlend : End_MRT(MRT_Deferred)", E_FAIL);

	if (FAILED(Blur_Emissive()))
		CHECK_MSG(L"Failed To CRenderer : Render_NonBlend : Blur_Emissive", E_FAIL);

	if (FAILED(Glow_Object()))
		CHECK_MSG(L"Failed To CRenderer : Render_NonBlend : Glow_Object", E_FAIL);
...
HRESULT CRenderer::Blur_Emissive()
{
	...
	if (FAILED(pPostProcessing->DownScale_Blur(m_pTarget_Manager->Get_SRV(L"Target_Emissive"), CPostProcessing::EMISSIVE)))
		CHECK_MSG(L"Failed To CRenderer : Blur_Emissive : DownScale_Blur", E_FAIL);
...
HRESULT CRenderer::Glow_Object()
{
	...
	if (FAILED(pPostProcessing->DownScale_Blur(m_pTarget_Manager->Get_SRV(L"Target_ObjectGlow"), CPostProcessing::OBJECTGLOW)))
		CHECK_MSG(L"Failed To CRenderer : Glow_Object : DownScale_Blur", E_FAIL);
...
HRESULT CRenderer::Render_LightAcc()
{
	if (nullptr == m_pTarget_Manager || nullptr == m_pLight_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_LightAcc : nullptr", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_MRT(m_pDeviceContext, TEXT("MRT_LightAcc"))))
		CHECK_MSG(L"Failed To CRenderer : Render_LightAcc : Begin_MRT(MRT_LightAcc)", E_FAIL);

	m_pLight_Manager->Render_Lights();

	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_LightAcc"))))
		CHECK_MSG(L"Failed To CRenderer : Render_LightAcc : End_MRT(MRT_LightAcc)", E_FAIL);
...
{
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : nullptr == m_pTarget_Manager", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_NonClear_MRT(m_pDeviceContext, TEXT("MRT_HDR"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Begin_NonClear_MRT(MRT_HDR)", E_FAIL);

	CPostProcessing* pPostProcessing = GET_INSTANCE(CPostProcessing);

	if (FAILED(m_pShader->Set_ShaderResourceView("g_DiffuseTexture", m_pTarget_Manager->Get_SRV(TEXT("Target_Diffuse")))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_ShaderResourceView(g_DiffuseTexture)", E_FAIL);
	if (FAILED(m_pShader->Set_ShaderResourceView("g_ShadeTexture", m_pTarget_Manager->Get_SRV(TEXT("Target_Shade")))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_ShaderResourceView(g_ShadeTexture)", E_FAIL);
	if (FAILED(m_pShader->Set_ShaderResourceView("g_SpecularTexture", m_pTarget_Manager->Get_SRV(TEXT("Target_Specular")))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_ShaderResourceView(g_SpecularTexture)", E_FAIL);
	if (FAILED(m_pShader->Set_ShaderResourceView("g_DepthTexture", m_pTarget_Manager->Get_SRV(TEXT("Target_Depth")))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_ShaderResourceView(g_DepthTexture)", E_FAIL);
	if (FAILED(m_pShader->Set_ShaderResourceView("g_LightDepthTexture", m_pTarget_Manager->Get_SRV(TEXT("Target_LightDepth")))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_ShaderResourceView(g_LightDepthTexture)", E_FAIL);
	if (FAILED(m_pShader->Set_ShaderResourceView("g_EmissiveTexture", m_pTarget_Manager->Get_SRV(TEXT("Target_Emissive")))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_ShaderResourceView(g_EmissiveTexture)", E_FAIL);
	if (FAILED(m_pShader->Set_ShaderResourceView("g_EmissiveBlurTexture", pPostProcessing->Get_DownScale4x4SRV(CPostProcessing::EMISSIVE))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_ShaderResourceView(g_EmissiveBlurTexture)", E_FAIL);
	if (FAILED(m_pShader->Set_ShaderResourceView("g_GlowTexture", pPostProcessing->Get_DownScale4x4SRV(CPostProcessing::OBJECTGLOW))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_ShaderResourceView(g_GlowTexture)", E_FAIL);

	RELEASE_INSTANCE(CPostProcessing);

	if (FAILED(m_pShader->Set_RawValue("g_WorldMatrix", &m_WorldMatrix, sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_WorldMatrix)", E_FAIL);
	if (FAILED(m_pShader->Set_RawValue("g_ViewMatrix", &m_ViewMatrix, sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_ViewMatrix)", E_FAIL);
	if (FAILED(m_pShader->Set_RawValue("g_ProjMatrix", &m_ProjMatrix, sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_ProjMatrix)", E_FAIL);
	...
	if (FAILED(m_pShader->Set_RawValue("g_ViewMatrixInv", &ViewMatrixInv, sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_ViewMatrixInv)", E_FAIL);
	if (FAILED(m_pShader->Set_RawValue("g_ProjMatrixInv", &ProjMatrixInv, sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_ProjMatrixInv)", E_FAIL);
	...
	if (FAILED(m_pShader->Set_RawValue("g_LightViewMatrix", &pPipeLine->Get_LightTransformFloat4x4_TP(CPipeLine::D3DTS_VIEW), sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_LightViewMatrix)", E_FAIL);
	if (FAILED(m_pShader->Set_RawValue("g_LightProjMatrix", &pPipeLine->Get_LightTransformFloat4x4_TP(CPipeLine::D3DTS_PROJ), sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_LightProjMatrix)", E_FAIL);

	_float fFar = pPipeLine->Get_CamFar();
	if (FAILED(m_pShader->Set_RawValue("g_fCamFar", &fFar, sizeof(_float))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_fCamFar)", E_FAIL);
	fFar = pPipeLine->Get_LightFar();
	if (FAILED(m_pShader->Set_RawValue("g_fLightFar", &fFar, sizeof(_float))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : Set_RawValue(g_fLightFar)", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_HDR"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Deferred : End_MRT(MRT_HDR)", E_FAIL);
...
{
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_NonLight : nullptr == m_pTarget_Manager", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_NonClear_MRT(m_pDeviceContext, TEXT("MRT_Effect"))))
		CHECK_MSG(L"Failed To CRenderer : Render_NonLight : Begin_NonClear_MRT(MRT_Effect)", E_FAIL);
	...
	for (auto& pGameObject : m_RenderList[GROUP_NONLIGHT])
	{
		if (nullptr != pGameObject)
		{
			if (FAILED(pGameObject->Render()))
				CHECK_MSG(L"Failed To CRenderer : Render_NonLight : Render", E_FAIL);
	...
	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_Effect"))))
		CHECK_MSG(L"Failed To CRenderer : Render_NonLight : End_MRT(MRT_Effect)", E_FAIL);
...
{
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_Blend : nullptr == m_pTarget_Manager", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_NonClear_MRT(m_pDeviceContext, TEXT("MRT_Effect"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Blend : Begin_NonClear_MRT(MRT_Effect)", E_FAIL);
	...
	for (auto& pGameObject : m_RenderList[GROUP_ALPHABLEND])
	{
		...
		if (FAILED(hr))
			CHECK_MSG(L"Failed To CRenderer : Render_Blend : hr", E_FAIL);
	}

	m_RenderList[GROUP_ALPHABLEND].clear();

	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_Effect"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Blend : End_MRT(MRT_Effect)", E_FAIL);
...
{
	if (nullptr == m_pTarget_Manager)
		CHECK_MSG(L"Failed To CRenderer : Render_Result : nullptr == m_pTarget_Manager", E_FAIL);

	if (FAILED(m_pTarget_Manager->Begin_NonClear_MRT(m_pDeviceContext, TEXT("MRT_Final"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Result : Begin_NonClear_MRT(MRT_Final)", E_FAIL);
	...
	if (FAILED(pPostProcessing->Combine_Resources(m_pVIBuffer, Matrix)))
		CHECK_MSG(L"Failed To CRenderer : Render_Result : Combine_Resources", E_FAIL);

	if (FAILED(m_pTarget_Manager->End_MRT(m_pDeviceContext, TEXT("MRT_Final"))))
		CHECK_MSG(L"Failed To CRenderer : Render_Result : End_MRT(MRT_Final)", E_FAIL);
...
HRESULT CRenderer::Render_Output()
{
	if (FAILED(m_pShader->Set_ShaderResourceView("g_Texture", m_pTarget_Manager->Get_SRV(TEXT("Target_Output")))))
		CHECK_MSG(L"Failed To CRenderer : Render_Output : Set_ShaderResourceView(g_Texture)", E_FAIL);

	if (FAILED(m_pShader->Set_RawValue("g_WorldMatrix", &m_WorldMatrix, sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Output : Set_RawValue(g_WorldMatrix)", E_FAIL);
	if (FAILED(m_pShader->Set_RawValue("g_ViewMatrix", &m_ViewMatrix, sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Output : Set_RawValue(g_ViewMatrix)", E_FAIL);
	if (FAILED(m_pShader->Set_RawValue("g_ProjMatrix", &m_ProjMatrix, sizeof(_float4x4))))
		CHECK_MSG(L"Failed To CRenderer : Render_Output : Set_RawValue(g_ProjMatrix)", E_FAIL);
...
HRESULT CRenderer::Render_UI()
{
	...
	for (auto& pGameObject : m_RenderList[GROUP_UI])
	{
		...
		if (FAILED(hr))
		{
			CHECK_MSG(L"Failed To CRenderer : Render_UI : hr", E_FAIL);
		}
	}
