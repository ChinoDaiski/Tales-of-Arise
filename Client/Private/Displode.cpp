#include "stdafx.h"
#include "..\Public\Displode.h"
#include "GameInstance.h"
#include "Animation.h"

CDisplode::CDisplode(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
	: CBullet(pDeviceOut, pDeviceContextOut)
{

}

CDisplode::CDisplode(const CDisplode & rhs)
	: CBullet(rhs)
{
}

HRESULT CDisplode::NativeConstruct_Prototype()
{
...
HRESULT CDisplode::NativeConstruct(void * pArg)
{
...
void CDisplode::Tick(_double TimeDelta)
{
...
void CDisplode::LateTick(_double TimeDelta)
{
...
HRESULT CDisplode::SetUp_Components()
{
	/* For.Com_Renderer */
...
HRESULT CDisplode::SetUp_ConstantTable()
{
	CGameInstance*		pGameInstance = GET_INSTANCE(CGameInstance);
...
CDisplode * CDisplode::Create(ID3D11Device* pDeviceOut, ID3D11DeviceContext* pDeviceContextOut)
{
	CDisplode*	pInstance = new CDisplode(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created CDisplode"));
...
CGameObject * CDisplode::Clone(void * pArg)
{
	CDisplode*	pInstance = new CDisplode(*this);
