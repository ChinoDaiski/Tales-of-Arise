#include "stdafx.h"
#include "..\Public\NotMovingFont.h"
...
NotMovingFont::NotMovingFont(ID3D11Device * pDeviceOut, ID3D11DeviceContext * pDeviceContextOut)
	: CGameObject(pDeviceOut, pDeviceContextOut)
{
}

NotMovingFont::NotMovingFont(const NotMovingFont & rhs)
	: CGameObject(rhs)
	, m_ProjMatrix(rhs.m_ProjMatrix)
{
}

HRESULT NotMovingFont::NativeConstruct_Prototype()
{
...
HRESULT NotMovingFont::NativeConstruct(void * pArg)
{
	...
	m_kind = ((MHDESC*)pArg)->kind;		//여기서 1을 집어넣음.
...
void NotMovingFont::Tick(_double TimeDelta)
{
...
void NotMovingFont::LateTick(_double TimeDelta)
{
...
HRESULT NotMovingFont::Render()
{
	for (int i = 0; i <m_str.size() ; ++i)
	{
		...
		//종류 별로 뺴주삼..
		m_kind = m_str.at(i) - '0';
		m_pTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_fX - g_iWinCX * 0.5f- ((m_str.size() -i) * 10)+40.f, -m_fY + g_iWinCY * 0.5f, 0.f, 1.f));
...
HRESULT NotMovingFont::SetUp_Components()
{
	/* For.Com_Renderer */
...
NotMovingFont * NotMovingFont::Create(ID3D11Device * pDeviceOut, ID3D11DeviceContext * pDeviceContextOut)
{
	NotMovingFont*	pInstance = new NotMovingFont(pDeviceOut, pDeviceContextOut);

	if (FAILED(pInstance->NativeConstruct_Prototype()))
	{
		MSG_BOX(TEXT("Failed to Created NotMovingFont"));
...
CGameObject * NotMovingFont::Clone(void * pArg)
{
	NotMovingFont*	pInstance = new NotMovingFont(*this);
