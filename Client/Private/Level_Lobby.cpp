...
HRESULT CLevel_Lobby::NativeConstruct()
{
	...
	if (FAILED(Ready_Dissolve(L"Layer_Dissolve")))
		return E_FAIL;
...
HRESULT CLevel_Lobby::Ready_Dissolve(const _tchar * pLayerTag)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);

	if (nullptr == pGameInstance->Add_GameObjectToLayer(LEVEL_STATIC, pLayerTag, L"Prototype_GameObject_Dissolve"))
		return E_FAIL;
