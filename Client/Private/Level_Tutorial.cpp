...
HRESULT CLevel_Tutorial::NativeConstruct()
{
	...
	if (FAILED(Ready_Lights("../Bin/Resources/Dat/Stage01/", "LightInfo.dat")))
		return E_FAIL;
	...
	if (FAILED(Ready_Layer_Fire_Deco(TEXT("Layer_FireDeco"))))
		return E_FAIL;
	...
	// 스카이 박스 정보 추가
	if (FAILED(Ready_Skybox("../Bin/Resources/Dat/Stage01/", "SkyboxInfo.dat")))
		return E_FAIL;

	// 불 정보 추가
	if (FAILED(Ready_MapFire("../Bin/Resources/Dat/Stage01/", "FireInfo.dat", LEVEL_TUTORIAL)))
		return E_FAIL;

	// 포스트 프로세싱 정보 추가
	if (FAILED(Ready_PostProcessing("../Bin/Resources/Dat/Stage01/", "PostProcessingInfo.dat")))
		return E_FAIL;
...
HRESULT CLevel_Tutorial::Ready_Lights(const char * pModelFilePath, const char * pModelFileName)
{
...
HRESULT CLevel_Tutorial::Ready_Skybox(const char * pModelFilePath, const char * pModelFileName)
{
	char szFullPath[MAX_PATH] = "";
...
HRESULT CLevel_Tutorial::Ready_MapFire(const char * pModelFilePath, const char * pModelFileName, LEVEL eLevel)
{
		...
		for (auto& Obj : Objlist) {
			...
			if (!_tcscmp(pMapObjectModelTag, pObjModelTag)) {
				...
				for (auto& Fire : FireInfo[iMapObjectCnt]) {
					...
					// 카메라 검색
					CLayer* pCameraLayer = pGameInstance->Find_Layer(eLevel, TEXT("Layer_Camera"));
...
HRESULT CLevel_Tutorial::Ready_PostProcessing(const char * pModelFilePath, const char * pModelFileName)
{
	char szFullPath[MAX_PATH] = "";
...
HRESULT CLevel_Tutorial::Ready_Layer_Fire_Deco(const _tchar * pLayerTag)
{
	CGameInstance* pGameInstance = GET_INSTANCE(CGameInstance);

	CFire_Deco::FIREDECODESC tFireDecoDesc;
	ZeroMemory(&tFireDecoDesc, sizeof(CFire_Deco::FIREDECODESC));

	tFireDecoDesc.fOriginalStrength = 1.f;
	tFireDecoDesc.fGlowStrength = 0.6f;
	tFireDecoDesc.iFrameSpeed = 32;
	tFireDecoDesc.iShaderPass = 2;
	tFireDecoDesc.tagTextureCom = L"Prototype_Component_Texture_Fire0";
	tFireDecoDesc.vLook = _float3(0.f, 0.f, 1.f);
	tFireDecoDesc.vPosition = _float3(-133.f, -10.f, -30.f);
	tFireDecoDesc.vScale = _float3(1.f, 1.f, 1.f);

	if (nullptr == pGameInstance->Add_GameObjectToLayer(LEVEL_TUTORIAL, pLayerTag, L"Prototype_GameObject_Fire", &tFireDecoDesc))
		return E_FAIL;
...
HRESULT CLevel_Tutorial::Ready_Map(const char * pModelFilePath, const char * pModelFileName, _vector vCenterPos)
{
	...
	while (true)
	{
		...
		if (!_tcscmp(Desc.pPrototype_ObjectName, TEXT("Prototype_Component_Model_Owl_001"))) {
			...
			COwl* pOwl = dynamic_cast<COwl*>(pGameInstance->Add_GameObjectToLayer(LEVEL_TUTORIAL, LAYER_MAPTOOL_Owls, TEXT("Prototype_GameObject_Owl"), &OwlDesc));
